 ...
 * 扫描入库(可指定日期)
 */
public class StockInWithDateActivity extends BaseActivity implements View.OnClickListener , OnViewClickListener {
...
//                ToastUtil.showToastLong("建议仓库存放:" + boxItemEntity.getIstName());
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_stock_in_with_date_layout);

        mRecyclerView = (RecyclerView) findViewById(R.id.rv_box_item_with_date);
        addTrayScannerButton = (Button) findViewById(R.id.btn_add_tray_scanner_with_date);
        addTrayPhotoButton = findViewById(R.id.btn_add_tray_photo_with_date);
        scanAreaButton = (Button) findViewById(R.id.btn_scan_area_with_date);
        warehouseInButton = (Button) findViewById(R.id.btn_exe_warehouse_in_with_date);
        inputEditText = (EditText) findViewById(R.id.input_EditText_with_date);

        stockSwitch = findViewById(R.id.setting_open_suggest_stock_Switch_with_date);
        switchLayout = findViewById(R.id.setting_open_suggest_stock_Layout_with_date);
...
//            }
            ...
            new IntentIntegrator(StockInWithDateActivity.this).setCaptureActivity(CustomScannerActivity.class).initiateScan();
//            parseScanResult("");
    ...
    private void showTimePickerDialog(String pickType) {
        if (timePickerManager == null) {
            timePickerManager = new TimePickerManager(StockInWithDateActivity.this);
            //john 2022-02-28
...
//            }
        ...
        timePickerManager
                .setOnViewClickListener(StockInWithDateActivity.this)
                .showDialog(pickType);
        AppUtil.forceHideInputMethod(StockInWithDateActivity.this);
    }
...
//                            CommonUtil.ShowToast(StockInActivity.this, "还没有扫描库位", R.mipmap.warning, Toast.LENGTH_SHORT);
            ...
            if (selectedcount > 0) {
                if (UserSingleton.get().getHRID() > 0 && !TextUtils.isEmpty(UserSingleton.get().getHRName())){
                    ...
                    CommAlertDialog.DialogBuilder builder = new CommAlertDialog.DialogBuilder(StockInWithDateActivity.this)
                        ...
                        public void onViewClick(Dialog dialog, View v, int tag) {
                            switch (tag) {
                                case CommAlertDialog.TAG_CLICK_LEFT:
                                    CommonUtil.doLogout(StockInWithDateActivity.this);
                                    dialog.dismiss();
                                    break;
                            }
                        }
...
//                ws_result = WebServiceUtil.op_Commit_DS_Item_Income_To_Warehouse(boxItemEntity,sql);
                ws_result = WebServiceUtil.op_Commit_DS_Item_Income_To_Warehouse_With_Date(boxItemEntity,scanCodeList.size() == selectedCount ? scanCodeList.get(0):"", warehouseInDate);
