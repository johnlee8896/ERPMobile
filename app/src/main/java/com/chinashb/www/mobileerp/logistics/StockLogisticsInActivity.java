...
//                ToastUtil.showToastLong("建议仓库存放:" + boxItemEntity.getIstName());
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mRecyclerView = (RecyclerView) findViewById(R.id.stock_logistics_rv_box_item);
        addTrayScannerButton = (Button) findViewById(R.id.stock_logistics_btn_add_tray_scanner);
        addTrayPhotoButton = findViewById(R.id.stock_logistics_btn_add_tray_photo);
        scanAreaButton = (Button) findViewById(R.id.stock_logistics_btn_scan_area);
        warehouseInButton = (Button) findViewById(R.id.stock_logistics_btn_exe_warehouse_in);
        inputEditText = (EditText) findViewById(R.id.stock_logistics_input_EditText);

        stockSwitch = findViewById(R.id.stock_logistics_setting_open_suggest_stock_Switch);
        switchLayout = findViewById(R.id.stock_logistics_setting_open_suggest_stock_Layout);
...
//            }
            ...
            new IntentIntegrator(StockLogisticsInActivity.this).setCaptureActivity(CustomScannerActivity.class).initiateScan();
//            parseScanResult("");
...
//                            CommonUtil.ShowToast(StockInActivity.this, "还没有扫描库位", R.mipmap.warning, Toast.LENGTH_SHORT);
            ...
            if (selectedcount > 0) {
                if (UserSingleton.get().getHRID() > 0 && !TextUtils.isEmpty(UserSingleton.get().getHRName())){
                    ...
                    CommAlertDialog.DialogBuilder builder = new CommAlertDialog.DialogBuilder(StockLogisticsInActivity.this)
