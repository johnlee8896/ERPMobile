...
import com.chinashb.www.mobileerp.bean.BoxLoadedQtyBean;
...
/***
 * @date 创建时间 2023/12/27 10:59 AM
 * @author 作者: liweifeng
 * @description 成品车间仓扫描出库
 */
public class BuWarehouseAccountOutActivity extends BaseActivity implements View.OnClickListener {

    @BindView(R.id.product_bu_warehouse_out_scan_button) Button scanBoxButton;
    @BindView(R.id.product_bu_warehouse_out_scan_area_button) Button scanAreaButton;
    @BindView(R.id.product_bu_warehouse_out_warehouse_in_button) Button warehouseOutButton;
    @BindView(R.id.product_bu_warehouse_out_input_EditText) EditText inputEditText;
    @BindView(R.id.product_bu_warehouse_out_recyclerView) CustomRecyclerView recyclerView;
    @BindView(R.id.product_bu_warehouse_out_select_wc_button) Button selectWcButton;
    @BindView(R.id.product_bu_warehouse_out_wc_name_textView) TextView wcNameTextView;
    @BindView(R.id.product_bu_warehouse_out_select_NO_button) Button selectNOButton;
    @BindView(R.id.product_bu_warehouse_out_NO_textView) TextView NOTextView;
    @BindView(R.id.product_bu_warehouse_out_item_info_textview) TextView itemInfoTextView;
    @BindView(R.id.product_bu_warehouse_out_ist_info_textview) TextView istInfoTextView;
    @BindView(R.id.product_bu_warehouse_out_qty_editText) EditText outQtyEditText;
    int workLineId = 0;
...
//    private ArrayList<Integer> boxIDList;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_product_bu_warehouse_out_layout);
        //保存所有已扫过的boxID，去重复判断
...
//        recyclerView.setLayoutManager(new LinearLayoutManager(this));//这里用线性显示 类似于listview
    ...
    private void setViewsListener() {
        ...
        warehouseOutButton.setOnClickListener(this);
...
//                    }else{
                    ...
                    hasScanItem = true;
                    String sql = "Select box_loaded_qty from code_box where box_id = " + boxId;
                    GetRealBoxQtyAsyncTask task = new GetRealBoxQtyAsyncTask();
                    task.execute(sql);
//                    }
...
//            }
            ...
            handleScanArea();
        } else if (view == warehouseOutButton) {
            handleOutWareHouse();
    ...
    private void handleSelectNO() {
        if (commonSelectInputDialog == null) {
            commonSelectInputDialog = new CommonSelectInputDialog(BuWarehouseAccountOutActivity.this);
        }
                ...
                public void onViewClick(Dialog dialog, View v, int tag) {
                    switch (tag) {
                        case CommAlertDialog.TAG_CLICK_LEFT:
                            CommonUtil.doLogout(BuWarehouseAccountOutActivity.this);
                            dialog.dismiss();
                            break;
                    }
                }
...
//            handleIntoWareHouse();
    ...
    private class ExeWarehouseProductOutCodeBoxAsyncTask extends AsyncTask<String, Void, Void> {
        WsResult ws_result;
...
//            ws_result = WebServiceUtil.op_Product_Manu_In_Bu_Warehouse_Account(boxId, thePlace.getIst_ID(), thePlace.getSub_Ist_ID(), remark);
            ...
            int entityID = 19;
            String entityName = "装配车间";
            if (UserSingleton.get().getUserInfo().getBu_ID() == 81){
                entityID = 357;
                entityName = "计划物控部";
            }
            ws_result = WebServiceUtil.op_Product_Manu_Out_Bu_Warehouse_Account(boxId,entityID,entityName,listNo,outQty);
        ...
        protected void onPostExecute(Void result) {
            ...
            if (ws_result != null) {
                if (ws_result.getResult()) {
                    ...
                    hasScanItem = false;
                    outQtyEditText .setHint("请输入出库数量，必须为整数");
                    inputEditText.setText("");
                }else {
                    ToastUtil.showToastShort("出现错误" + ws_result.getErrorInfo());
                }

            }else {
                    ToastUtil.showToastShort("出库错误" );
    ...
    private class GetRealBoxQtyAsyncTask extends AsyncTask<String, Void, BoxLoadedQtyBean> {

        @Override
        protected BoxLoadedQtyBean doInBackground(String... params) {
            if (params != null && params.length > 0) {
                ...
                if (result != null && result.getResult()) {
                    String jsonData = result.getErrorInfo();
                    Gson gson = new Gson();
                    List<BoxLoadedQtyBean> boxItemEntityList = gson.fromJson(jsonData, new TypeToken<List<BoxLoadedQtyBean>>() {
                    }.getType());
        ...
        @Override
        protected void onPostExecute(BoxLoadedQtyBean result) {
            if (result != null) {
                String text = itemInfoTextView.getText().toString();
