 ...
 * @description 外贸发运扫码管理-新装托流程
 */
public class NewAllShipmentAccountDeliveryManagement extends BaseActivity {

    // created by code-x John
    // start: 2026-05-21 10:28:18 CST
    // end: 2026-05-21 10:28:18 CST
    private static final int INPUT_MIN_LENGTH = 3;
    private static final int MAX_RECENT_RECORD_COUNT = 40;

    @BindView(R.id.new_shipment_attach_shipment_container_button)
    Button attachShipmentContainerButton;
    @BindView(R.id.new_shipment_detach_shipment_container_button)
    Button detachShipmentContainerButton;
    @BindView(R.id.new_shipment_attach_container_pallet_button)
    Button attachContainerPalletButton;
    @BindView(R.id.new_shipment_detach_container_pallet_button)
    Button detachContainerPalletButton;
    @BindView(R.id.new_shipment_add_pallet_item_button)
    Button addPalletItemButton;
    @BindView(R.id.new_shipment_remove_pallet_item_button)
    Button removePalletItemButton;
    @BindView(R.id.new_shipment_status_text)
    TextView statusText;
    @BindView(R.id.new_shipment_input_edit)
    EditText inputEdit;
    @BindView(R.id.new_shipment_camera_scan_button)
    Button cameraScanButton;
    @BindView(R.id.new_shipment_scan_list)
    CustomRecyclerView scanListView;
    @BindView(R.id.new_shipment_toggle_pallet_check_button)
    Button togglePalletCheckButton;
    @BindView(R.id.new_shipment_reset_target_button)
    Button resetTargetButton;
    @BindView(R.id.new_shipment_clear_records_button)
    ...
    private NewShipmentMode currentMode = NewShipmentMode.SHIPMENT_ATTACH_CONTAINER;
    ...
    private String currentScanBoxJson = "";
    private TradePalletScanBoxBean currentScanBoxBean;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_new_all_shipment_account_delivery_management);
    ...
    @OnClick({
            R.id.new_shipment_attach_shipment_container_button,
            R.id.new_shipment_detach_shipment_container_button,
            R.id.new_shipment_attach_container_pallet_button,
            R.id.new_shipment_detach_container_pallet_button,
            R.id.new_shipment_add_pallet_item_button,
            R.id.new_shipment_remove_pallet_item_button
    })
    public void onModeButtonClick(View view) {
        switch (view.getId()) {
            case R.id.new_shipment_attach_shipment_container_button:
                switchMode(NewShipmentMode.SHIPMENT_ATTACH_CONTAINER);
                break;
            case R.id.new_shipment_detach_shipment_container_button:
                switchMode(NewShipmentMode.SHIPMENT_DETACH_CONTAINER);
                break;
            case R.id.new_shipment_attach_container_pallet_button:
                switchMode(NewShipmentMode.CONTAINER_ATTACH_PALLET);
                break;
            case R.id.new_shipment_detach_container_pallet_button:
                switchMode(NewShipmentMode.CONTAINER_DETACH_PALLET);
                break;
            case R.id.new_shipment_add_pallet_item_button:
                switchMode(NewShipmentMode.PALLET_ADD_BOX);
                break;
            case R.id.new_shipment_remove_pallet_item_button:
                switchMode(NewShipmentMode.PALLET_REMOVE_BOX);
                break;
            default:
                break;
        }
    ...
    @OnClick(R.id.new_shipment_camera_scan_button)
    ...
    @OnClick(R.id.new_shipment_reset_target_button)
    ...
    @OnClick(R.id.new_shipment_toggle_pallet_check_button)
    ...
    @OnClick(R.id.new_shipment_clear_records_button)
    ...
    private void switchMode(NewShipmentMode mode) {
        currentMode = mode;
        refreshUi();
        focusInput();
    }
    ...
    private void handleShipmentContainerScan(String scan) {
        ...
        isParsingScan = true;
        new ShipmentContainerActionTask(scan, currentMode == NewShipmentMode.SHIPMENT_ATTACH_CONTAINER).execute();
    }
    ...
    private void handleContainerPalletScan(String scan) {
        ...
        isParsingScan = true;
        new ContainerPalletActionTask(scan, currentMode == NewShipmentMode.CONTAINER_ATTACH_PALLET).execute();
    }
    ...
    private void handlePalletItemScan(String scan) {
        ...
        isParsingScan = true;
        new PalletItemActionTask(scan, currentMode == NewShipmentMode.PALLET_ADD_BOX).execute();
    }
    ...
    private void bindPallet(String scan, TradePalletBean palletBean) {
        ...
        currentScanBoxJson = "";
        currentScanBoxBean = null;
        ensureGroupExists(palletItemMap, scan);
    }

    private void bindScanBox(String scanBoxJson, TradePalletScanBoxBean scanBoxBean) {
        currentScanBoxJson = scanBoxJson;
        currentScanBoxBean = scanBoxBean;
    }
    ...
    private void refreshModeButtons() {
        attachShipmentContainerButton.setSelected(currentMode == NewShipmentMode.SHIPMENT_ATTACH_CONTAINER);
        detachShipmentContainerButton.setSelected(currentMode == NewShipmentMode.SHIPMENT_DETACH_CONTAINER);
        attachContainerPalletButton.setSelected(currentMode == NewShipmentMode.CONTAINER_ATTACH_PALLET);
        detachContainerPalletButton.setSelected(currentMode == NewShipmentMode.CONTAINER_DETACH_PALLET);
        addPalletItemButton.setSelected(currentMode == NewShipmentMode.PALLET_ADD_BOX);
        removePalletItemButton.setSelected(currentMode == NewShipmentMode.PALLET_REMOVE_BOX);
    ...
    private String buildStatusText() {
        ...
        if (currentMode == NewShipmentMode.PALLET_ADD_BOX || currentMode == NewShipmentMode.PALLET_REMOVE_BOX) {
            builder.append("最近包装解析：").append(currentScanBoxBean == null ? "-" : formatScanBoxDetail(currentScanBoxBean)).append("\n");
        }
        builder.append("下一步：");
    ...
    private void buildDisplayList() {
        scanList.clear();
        scanList.add(new ShipmentDisplayEntity("当前模式", currentMode.getModeTitle(),
                hasPrimaryTargetForMode() ? currentMode.getWaitingSecondaryHint() : currentMode.getWaitingPrimaryHint()));
    ...
    private void appendCurrentTargetGroup() {
        switch (currentMode) {
            ...
            case PALLET_REMOVE_BOX:
                ...
                if (currentScanBoxBean != null) {
                    scanList.add(new ShipmentDisplayEntity("当前包装解析", safe(currentScanBoxJson), formatScanBoxDetail(currentScanBoxBean)));
                }
                break;
            default:
                break;
    ...
    private TradePalletScanBoxBean parseScanBoxBean(String scanBoxJson) {
        if (TextUtils.isEmpty(scanBoxJson)) {
            return null;
        }
        return JsonUtil.parseJsonToObject(scanBoxJson, TradePalletScanBoxBean.class);
    }

    private String formatShipmentDetail(TradeShipmentBean bean) {
        return "TrackNo:" + safe(bean.getTrackNo()) + "  Delivery_ID:" + bean.getDelivery_ID() + "  日期:" + safe(bean.getDelivery_Date());
    }

    private String formatContainerDetail(TradeContainerBean bean) {
        return "箱号:" + safe(bean.getContainer_No()) + "  规格:" + safe(bean.getContainer_Type()) + "  Seal:" + safe(bean.getSeal_No());
    }

    private String formatPalletDetail(TradePalletBean bean) {
        return "托盘号:" + safe(bean.getPallet_SerialNo()) + "  已检:" + (bean.isChecked() ? "是" : "否") + "  可发运:" + (bean.isLoadForDelivery() ? "是" : "否");
    }

    private String formatScanBoxDetail(TradePalletScanBoxBean bean) {
        return "Level:" + bean.getBoxLevel() + "-" + safe(bean.getBoxLevelName())
                + "  DIII_ID:" + bean.getDIII_ID()
                + "  Qty:" + bean.getQty()
                + "  Item_ID:" + bean.getItem_ID();
    }
    ...
    private class ShipmentTaskResult {
        ...
        TradePalletScanBoxBean scanBoxBean;
        String scan;
        String scanBoxJson;
    }
    ...
    private class ShipmentContainerActionTask extends AsyncTask<String, Void, ShipmentTaskResult> {
        ...
        protected void onPostExecute(ShipmentTaskResult result) {
            ...
            if (result.wsResult != null && result.wsResult.getResult()) {
                ...
                addRecentRecord(attach ? "发运单装箱" : "发运单移箱", result.scan, "目标发运单：" + currentShipmentScan);
                refreshUi();
    ...
    private class ContainerPalletActionTask extends AsyncTask<String, Void, ShipmentTaskResult> {
        ...
        protected void onPostExecute(ShipmentTaskResult result) {
            ...
            if (result.wsResult != null && result.wsResult.getResult()) {
                ...
                addRecentRecord(attach ? "集装箱装托" : "集装箱移托", result.scan, "目标集装箱：" + currentContainerScan);
                refreshUi();
    ...
    private class PalletItemActionTask extends AsyncTask<String, Void, ShipmentTaskResult> {
        ...
        protected ShipmentTaskResult doInBackground(String... strings) {
            ...
            try {
                if (add) {
                    WsResult scanResult = ensureWsResult(
                            WebServiceUtil.opTradePalletScanItem(UserSingleton.get().getHRID(), UserSingleton.get().getHRName(), itemScan),
                            "托盘包装扫码解析无返回结果");
                    if (!scanResult.getResult()) {
                        result.wsResult = scanResult;
                        return result;
                    }
                    result.scanBoxJson = scanResult.getInfo();
                    result.scanBoxBean = parseScanBoxBean(result.scanBoxJson);
                    result.wsResult = ensureWsResult(
                            WebServiceUtil.opTradePalletAddScanBox(UserSingleton.get().getHRID(), UserSingleton.get().getHRName(), currentPalletScan, result.scanBoxJson),
                            "托盘装包装无返回结果");
                } else {
                    result.wsResult = ensureWsResult(
                ...
                result.wsResult = new WsResult();
                result.wsResult.setResult(false);
                result.wsResult.setErrorInfo("包装操作失败：" + e.getMessage());
            }
            return result;
        }
        ...
        protected void onPostExecute(ShipmentTaskResult result) {
            isParsingScan = false;
            if (result.wsResult != null && result.wsResult.getResult()) {
                if (add && result.scanBoxBean != null) {
                    bindScanBox(result.scanBoxJson, result.scanBoxBean);
                }
                updateRelationMap(palletItemMap, currentPalletScan, result.scan, add);
                addRecentRecord(add ? "托盘装包装" : "托盘移包装", result.scan,
                        add && result.scanBoxBean != null
                                ? "目标托盘：" + currentPalletScan + "  " + formatScanBoxDetail(result.scanBoxBean)
                                : "目标托盘：" + currentPalletScan);
                refreshUi();
                showSuccess((add ? "包装加入成功：" : "包装移出成功：") + result.scan);
            } else {
                showError(result.wsResult == null ? "包装操作失败" : result.wsResult.getErrorInfo());
            }
        }
    }
