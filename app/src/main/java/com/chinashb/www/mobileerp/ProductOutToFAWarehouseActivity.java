...
/***
 * @date 创建时间 8/23/25 4:08 PM
 * @author 作者: liweifeng
 * @description 整托出给车间仓，与返工出给产线区别在入了车间仓？
 */

public class ProductOutToFAWarehouseActivity extends BaseActivity implements View.OnClickListener {

    @BindView(R.id.product_out_to_fa_select_wc_button) Button selectWcButton;
    @BindView(R.id.product_out_to_fa_wc_name_textView) TextView wcNameTextView;
    //    @BindView(R.id.product_out_to_fa_select_NO_button) Button selectNOButton;
//    @BindView(R.id.product_out_to_fa_NO_textView) TextView NOTextView;
    @BindView(R.id.product_out_to_fa_scan_button) Button scanBoxButton;
    @BindView(R.id.product_out_to_fa_warehouse_out_button) Button returnOutButton;
    @BindView(R.id.product_out_to_fa_input_EditText) EditText inputEditText;
    @BindView(R.id.product_out_to_fa_recyclerView) CustomRecyclerView recyclerView;
    @BindView(R.id.product_out_to_fa_product_info_textView) TextView itemInfoTextView;
...
//                NOTextView.setText((CharSequence) t);
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_product_box_out_to_fa_layout);
...
//                            currentPalletID = palletID;
    ...
    private void handleWCEntityListDialog(List<WCProductBean> productWCBeanList) {
        if (commonSelectInputDialog == null) {
            commonSelectInputDialog = new CommonSelectInputDialog(this);
        }
...
//            handleProductReturnOut();
            handleProductToFAWarehouse();

        }
    }

    private void handleProductToFAWarehouse() {
        if (!hasScanItem){
            ToastUtil.showToastShort("请先扫描标签！");
        }
        if (hasScanItem){
            if (UserSingleton.get().getHRID() > 0 && !TextUtils.isEmpty(UserSingleton.get().getHRName())) {
                ExeWarehouseProductOutToFAWarehouseAsyncTask task = new ExeWarehouseProductOutToFAWarehouseAsyncTask();
                task.execute();
                    ...
                    public void onViewClick(Dialog dialog, View v, int tag) {
                        switch (tag) {
                            case CommAlertDialog.TAG_CLICK_LEFT:
                                CommonUtil.doLogout(ProductOutToFAWarehouseActivity .this);
                                dialog.dismiss();
                                break;
                        }
                    }
...
//                    "生产线", UserSingleton.get().getUserInfo().getBu_ID());
    ...
    private class ExeWarehouseProductOutToFAWarehouseAsyncTask extends AsyncTask<String, Void, Void> {
...
//                        ws_result = WebServiceUtil.opProductManuOutReturn(boxIDList.get(i), currentWCID, currentWCName, "");
                    ws_result = WebServiceUtil.opProductToFAWarehouse(boxIDList.get(i), 0, 0);
        ...
        protected void onPostExecute(Void result) {
            if (ws_result != null) {
                if (!ws_result.getResult()) {
                    ...
                    CommonUtil.ShowToast(ProductOutToFAWarehouseActivity.this, ws_result.getErrorInfo(), R.mipmap.warning);

                } else {
                    CommonUtil.ShowToast(ProductOutToFAWarehouseActivity.this, "操作成功！",R.mipmap.smiley);
