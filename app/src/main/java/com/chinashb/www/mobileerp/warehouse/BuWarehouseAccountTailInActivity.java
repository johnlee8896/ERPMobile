...
/***
 * @date 创建时间 2023/10/24 6:46 PM
 * @author 作者: liweifeng  2026/03/31 重新修改
 * @description 成品车间仓入库，主要处理尾数
 */
public class BuWarehouseAccountTailInActivity extends BaseActivity implements View.OnClickListener {
    
    @BindView(R.id.product_bu_warehouse_in_scan_button) Button scanBoxButton;
    @BindView(R.id.product_bu_warehouse_in_scan_area_button) Button scanAreaButton;
    @BindView(R.id.product_bu_warehouse_in_warehouse_in_button) Button warehouseInButton;
    @BindView(R.id.product_bu_warehouse_in_input_EditText) EditText inputEditText;
    @BindView(R.id.product_bu_warehouse_in_recyclerView) CustomRecyclerView recyclerView;
    @BindView(R.id.product_bu_warehouse_in_select_wc_button) Button selectWcButton;
    @BindView(R.id.product_bu_warehouse_in_wc_name_textView) TextView wcNameTextView;
    @BindView(R.id.product_bu_warehouse_in_select_NO_button) Button selectNOButton;
    @BindView(R.id.product_bu_warehouse_in_NO_textView) TextView NOTextView;
    @BindView(R.id.product_bu_warehouse_in_item_info_textview) TextView itemInfoTextView;
    @BindView(R.id.product_bu_warehouse_in_ist_info_textview) TextView istInfoTextView;
    int workLineId = 0;
...
//            getWCList();
    ...
    private void handleSelectNO() {
        if (commonSelectInputDialog == null) {
            commonSelectInputDialog = new CommonSelectInputDialog(BuWarehouseAccountTailInActivity.this);
        }
    ...
    private void handleScanArea() {
        if (hasScanItem) {
            ...
            ToastUtil.showToastShort("没有扫描箱码或尾数仓库位置码没有成功，请重新扫描！");
        }
...
//        C-20191226-01
        for (int i = 0; i < 15; i++) {
            noList.add(String.format("C-W-%s-%s", UnitFormatUtil.formatTimeToDayWithoutLine(System.currentTimeMillis()), String.format("%02d", i + 1)));
        }
        return noList;
    }
...
//                task.execute();
                ExeWarehouseProductFATailInCodeBoxAsyncTask task = new ExeWarehouseProductFATailInCodeBoxAsyncTask();
                task.execute();
            } else {
                ...
                public void onViewClick(Dialog dialog, View v, int tag) {
                    switch (tag) {
                        case CommAlertDialog.TAG_CLICK_LEFT:
                            CommonUtil.doLogout(BuWarehouseAccountTailInActivity.this);
                            dialog.dismiss();
                            break;
                    }
                }
                ...
                if (!ws_result.getResult()) {
                    ...
                    if (!TextUtils.isEmpty(errorInfo)) {
                        if (errorInfo.contains("月份与批次") && errorInfo.contains("不一致")) {
                            CommAlertDialog.DialogBuilder builder = new CommAlertDialog.DialogBuilder(BuWarehouseAccountTailInActivity.this)
...
//                                            //直接入库
                            ...
                            CommonUtil.ShowToast(BuWarehouseAccountTailInActivity.this, ws_result.getErrorInfo(), R.mipmap.warning);
                        }
                    }
...
//                    CommonUtil.ShowToast(ProductScanBoxInActivity.this, "入库完成" + ws_result.getErrorInfo(), R.mipmap.smiley);
                    CommonUtil.ShowToast(BuWarehouseAccountTailInActivity.this, "入库完成", R.mipmap.smiley);
                    hasScanItem = false;
                    inputEditText.setText("");
...
//            mRecyclerView.setAdapter(boxItemAdapter);
    ...
    private class DirectExeWarehouseProductInCodeBoxAsyncTask extends AsyncTask<String, Void, Void> {
        ...
        protected Void doInBackground(String... params) {
            ...
            ws_result = WebServiceUtil.op_Product_Manu_In_Pallet_Neglect_Month(boxId, thePlace.getIst_ID(), thePlace.getSub_Ist_ID(), remark);
...
//                    if (!TextUtils.isEmpty(errorInfo)) {
//                        if (errorInfo.contains("月份与批次") && errorInfo.contains("不一致")) {
//                            CommAlertDialog.DialogBuilder builder = new CommAlertDialog.DialogBuilder(BuWarehouseAccountInActivity.this)
//                                    .setTitle("").setMessage("当前托盘号月份与批次月份不一致，是否修改月份，点确定为修改，点取消则直接入库！")
//                                    .setLeftText("确定修改")
//                                    .setRightText("直接入库");
//
...
//                            builder.create().show();
//                        } else {
//                            CommonUtil.ShowToast(BuWarehouseAccountInActivity.this, ws_result.getErrorInfo(), R.mipmap.warning);
//                        }
//                    }
                    CommonUtil.ShowToast(BuWarehouseAccountTailInActivity.this, ws_result.getErrorInfo(), R.mipmap.warning);
...
//            mRecyclerView.setAdapter(boxItemAdapter);
    ...
    private class ModifyMonthExeWarehouseProductInCodeBoxAsyncTask extends AsyncTask<String, Void, Void> {
        ...
        protected Void doInBackground(String... params) {
            ...
            ws_result = WebServiceUtil.op_Product_Manu_In_Pallet_Modify_Month(boxId, thePlace.getIst_ID(), thePlace.getSub_Ist_ID(), remark,month);
...
//    }
    ...
    private class ExeWarehouseProductFATailInCodeBoxAsyncTask extends AsyncTask<String, Void, Void> {
        WsResult ws_result;
...
//            车间仓尾数入库
//            ws_result = WebServiceUtil.op_Product_Manu_In_Bu_Warehouse_Account(boxId, thePlace.getIst_ID(), thePlace.getSub_Ist_ID(), remark);
            ws_result = WebServiceUtil.op_Scan_Product_To_Fractional_Library(boxId, thePlace.getIst_ID(), thePlace.getSub_Ist_ID(), remark);
        ...
        protected void onPostExecute(Void result) {
            if (ws_result != null) {
                if (!ws_result.getResult()) {
                    ...
                    //Toast.makeText(StockInActivity.this,"入库完成",Toast.LENGTH_LONG).show();
//                    CommonUtil.ShowToast(ProductScanBoxInActivity.this, "入库完成" + ws_result.getErrorInfo(), R.mipmap.smiley);
                    CommonUtil.ShowToast(BuWarehouseAccountTailInActivity.this, "扫描尾数入库完成", R.mipmap.smiley);
                    hasScanItem = false;
                    inputEditText.setText("");
                    itemInfoTextView.setText("物料信息");
                    istInfoTextView.setText("入库区域");
...
//                    }
                ...
                CommonUtil.ShowToast(BuWarehouseAccountTailInActivity.this, "扫描尾数入库失败！", R.mipmap.warning);
