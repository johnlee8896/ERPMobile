...
public final class APILog {
    ...
    private static final String TAG_DEFAULT = "APILog_";
    private static final String SUFFIX = ".java";
    private static final int JSON = 0x7;

    private static final int SYSO = 0x8;
    private static final int HTTP_LOG = 0x9;
    ...
    public static void httpLog(List<String> logList) {
        printLog(HTTP_LOG, logList);
    }
    ...
    private static void printLog(int type, Object... objects) {
        ...
        switch (type) {
            ...
            case JSON:
                ...
                break;
            case HTTP_LOG:
                printHttpLog(tag, headString, objects);
                break;
        }
    }
...
//    }
    ...
    private static final int logMaxCount = 3600;

    public static void printHttpLog(String tag, String headString, Object... objects) {
        if (objects != null) {
            ArrayList<String> logList = ((ArrayList<String>) objects[0]);
            ...
            for (String line : logList) {
                if (line.length() > logMaxCount) {
                    int i = 1;
                    ArrayList<String> subList = new ArrayList<>();
                    while (line.length() > logMaxCount) {
                        Log.i(tag, String.format(" ~~ %d ~~║ %s", i++, line.substring(0, logMaxCount)));
                        line = line.substring(logMaxCount);
                    }
                    if (line.length() > 0) {
                        Log.i(tag, String.format(" ~~ %d ~~║ %s", i, line));
                    }
                } else {
                    Log.i(tag, "║ " + line);
                }
